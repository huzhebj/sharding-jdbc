 ...
 */
public class StudentDbShardingAlgorithm implements SingleKeyDatabaseShardingAlgorithm<Integer> {
    @Override
    public String doEqualSharding(Collection<String> dbNames, ShardingValue<Integer> shardingValue) {
        for (String dbName : dbNames) {
            if (dbName.endsWith(shardingValue.getValue() % 5 + "")) {
                return dbName;
            }
        }
